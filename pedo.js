...
  // ============================================
  // OBTENER DATOS DEL SERVIDOR (NUEVA FUNCIÓN)
  // ============================================
  /**
   * Realiza una solicitud GET al servidor para un sensor específico.
   * Utiliza el 'timeout' nativo de Axios para manejar respuestas lentas.
   * @param {number} sensorId - El ID del sensor a consultar (1, 2 o 3).
   */
  const realizarSolicitud = async (sensorId) => {
    const url = `${CONFIG.URL_SERVIDOR}?sensor_id=${sensorId}`;
    
    try {
      // Usamos el timeout nativo de Axios (2000ms = 2 segundos) y quitamos headers
      const response = await axios.get(url, { timeout: 2000 });
      
      const data = response.data;
      
      // Lógica robusta para obtener la distancia y el objeto
...
        console.error(`❌ Timeout sensor ${sensorId} después de 2000ms`);
      } else if (error.response) {
        // Error de servidor (4xx o 5xx)
        console.error(`❌ Error HTTP sensor ${sensorId} (Status: ${error.response.status}):`, error.response.data);
      } else {
        // Error de red o petición
        console.error(`❌ Error de conexión sensor ${sensorId}:`, error.message);
      }
      
      // Opcional: Establecer un valor de error o N/A en caso de fallo
      setSensores((prev) => ({
        ...prev,
...
  // ============================================
  const monitoreoContinuo = async () => {
    const sensoresAProcesar = [1, 2, 3];
    const pausaEntreSolicitudes = 300; // ms

    for (const sensorId of sensoresAProcesar) {
      await realizarSolicitud(sensorId);
      // Esperar un momento antes de la siguiente solicitud para evitar el error 405 (sobrecarga)
      await new Promise(resolve => setTimeout(resolve, pausaEntreSolicitudes));
    }
  };
...
      }
    });
    console.log("🎲 Datos Aleatorios Generados", {sensores});
  };
...
    
    // Convertir 'monitoreoContinuo' en una dependencia estable o memoizar
    // Para evitar warnings de React y re-renders innecesarios.
    // Nota: La función 'monitoreoContinuo' se define sin dependencias externas, 
    // por lo que no necesita 'useCallback' a menos que 'realizarSolicitud' cambie
    // o se defina fuera del componente. En este caso, la dependencia se mantiene para 
    // compatibilidad con linters, pero se comporta de forma estable.
...
      
      // El intervalo llama a la función de monitoreo secuencial
      const interval = setInterval(monitoreoContinuo, INTERVALO_REFRESH);

      return () => clearInterval(interval);
    }
  }, [monitoreoContinuo]); // Dejar como dependencia si React Hook Linter está activo

  // [RESTO DE LAS FUNCIONES Y RENDER NO MODIFICADAS]
  // ... (obtenerColor, calcularPosicion, y todo el JSX y estilos permanecen igual)
...
    // Distancia 0 = muy cerca del coche (arriba, cerca del 25%)
    // Distancia 250 = lejos (abajo, 85%)
...
          
          // MOSTRAR SIEMPRE SI LA DISTANCIA ES MENOR A 250 O ES N/A
...
// El resto de los componentes Coche y ObjetoVehiculo y el objeto estilos no han sido modificados
// ...
// ...
